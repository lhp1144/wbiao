<template>
    <div class="EditAddress">
        ...
        <div class="address_warp">
           ...
           <!-- 省市区选择弹窗 -->
            <van-popup v-model="showAddress" position="bottom" :style="{ height: '35%' } ">
...
export default {
    ...
    data() {
        return {
            ...
            showAddress: false,
             userlist: {
                u_id: Math.floor(Math.random()*1000),
                //收货人
                username: "",
                //手机号
                phone: "",
    ...
    mounted() {
        ...
        // 挂载点击编辑按钮获取的地址，渲染到页面中
        let edit = JSON.parse(sessionStorage.getItem('editList')) || [];
        this.editArr = edit;
        console.log(this.editArr);
        for (let edit of this.editArr) {
            this.$refs.consignee.value = edit.name;
            this.$refs.phone.value = edit.phone;
            this.$refs.location.value = edit.address;
            this.$refs.detailAddress.value = edit.detailAddress;
        }
    },
    methods: {
        toAddress() {
        ...
        saveAddress() {
            ...
            // 临时地址
            let finishData = JSON.parse(sessionStorage.getItem('editList')) || [];
            this.finishArr = finishData;
            // 用户地址
            ...
            this.userArr.forEach((user, index) =>{
                this.finishArr.forEach((finish, index) =>{
                    if(finish.user_id === user.user_id){
                        let NewUserAddr = {
                            user_id: this.userlist.u_id,
                            name: user.name = newUser.name,
                            phone: user.phone = newUser.phone,
                            address: user.address = newUser.address,
                            detailAddress: user.detailAddress = newUser.detailAddress
                        }
                        // 删掉原来的那一条地址
                        let position = this.userArr.indexOf(user);
                        this.userArr.splice(position, 1);
                        this.userArr.unshift(NewUserAddr);
                        localStorage.setItem('userList', JSON.stringify(this.userArr));
                    }
                })
            });
            // sessionStorage.setItem('editList', JSON.stringify(this.finishArr));
            this.$router.push({path: '/Address'});
        }
    },
