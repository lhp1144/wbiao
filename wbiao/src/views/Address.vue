...
export default {
    ...
    methods: {
        ...
        edit(address) {
            ...
            // 将当前点击的address推进editAddress
            this.editAddress.push(address);
            // for (let editData of this.editAddress) {
            //    if(editData.user_id === editData.user_id){
            //        let position = this.editAddress.indexOf(address);
            //        this.editAddress.splice(position, 1);
            //    }else{
            //        this.editAddress.push(address);
            //    }
            // }
            sessionStorage.setItem('editList', JSON.stringify(this.editAddress));
        }
    },
